    ...
    ui(new Ui::ClientManagerForm)
{
    ...
    QFile file("clientlist.txt");               // 파일 불러오기
...
void ClientManagerForm::removeItem()        // 아이템 제거
{
    ...
    if(item != nullptr) {                                       // 아이템 내용이 있으면
        clientList.remove(item->text(0).toInt());               // 아이템의 아이디 삭제, 항목 다 삭제되는 듯
...
void ClientManagerForm::on_searchPushButton_clicked()
{
    ...
    // i가 0이 아니면 검색의 대소문자 구분하거나 검색어가 항목에 포함, 0이면 검색의 대소문자 구분???
        ...
        {
            ClientItem* c = static_cast<ClientItem*>(i);        // 검색으로 찾은 항목을 매칭시킴
            int id = c->id();                                   // 항목의 아이디를 가져옴
            QString name = c->getName();                        // 이름
            QString number = c->getPhoneNumber();               // 전화번호
            QString address = c->getAddress();                  // 주소
            ClientItem* item = new ClientItem(id, name, number, address);       // 검색으로 찾은 항목을 다시한번 매칭
...
void ClientManagerForm::on_modifyPushButton_clicked()               // 커넥트 없어도 자동으로 연결됨. qt문법
{
    ...
    if(item != nullptr) {
        ...
        ClientItem* c = clientList[key];                            // 아이디에 해당하는 값 매칭
        QString name, number, address;                              // 이름, 전화번호, 주소
        name = ui->nameLineEdit->text();                            // 라인에딧에 이름쓴거 name에 저장
        number = ui->phoneNumberLineEdit->text();                   // 라인에딧에 번호쓴거 number에 저장
        address = ui->addressLineEdit->text();                      // 라인에딧에 주소쓴거 address에 저장
        c->setName(name);                                           // 이름 수정
        c->setPhoneNumber(number);                                  // 전화번호 수정
        c->setAddress(address);                                     // 주소 수정
        clientList[key] = c;                                        // clientList에 수정한 값 저장
    }
}
...
void ClientManagerForm::on_addPushButton_clicked()                  // 커넥트 없어도 자동으로 연결됨. qt문법
{
    ...
    address = ui->addressLineEdit->text();                          // 라인에딧에 주소쓴거 address에 저장
    if(name.length()) {
        ClientItem* c = new ClientItem(id, name, number, address);      // 각각의 값 매칭
        clientList.insert(id, c);                                       // clientList에 아이디에 해당하는 값 삽입
        ui->treeWidget->addTopLevelItem(c);                             // 탑레벨로 옮김
    }
}

void ClientManagerForm::on_treeWidget_itemClicked(QTreeWidgetItem *item, int column)    // 커넥트 없어도 자동으로 연결됨. qt문법
{
    Q_UNUSED(column);                                           // 경고메세지 안띄우게 하는 것 같음
    ui->idLineEdit->setText(item->text(0));                     // 아이디 재설정
    ui->nameLineEdit->setText(item->text(1));                   // 이름 재설정
    ui->phoneNumberLineEdit->setText(item->text(2));            // 전화번호 재설정
    ui->addressLineEdit->setText(item->text(3));                // 주소 재수정
}

ClientItem* ClientManagerForm::search(int id)
{
    auto it = clientList.find(id);
    return (it != clientList.end() ? clientList[id] : nullptr);
}
