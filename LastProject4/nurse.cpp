...
//------------------------------------------------------------ 데이터 전송 ---------------------------------------------------------------//
    ...
    connect(ui->Patlent_Gender_Male_pushButton, SIGNAL(clicked()), SLOT(Patlent_Gender_Male_pushButton_clicked()));
    connect(ui->Patlent_Gender_Female_pushButton, SIGNAL(clicked()), SLOT(Patlent_Gender_Female_pushButton_clicked()));
    connect(ui->Patlent_Gender_None_pushButton, SIGNAL(clicked()), SLOT(Patlent_Gender_None_pushButton_clicked()));
...
void Nurse::receiveData( )                                                               // 채팅자 끼리 채팅 주고 받기
{
    ...
    QByteArray bytearray[11];                                                                 // 소켓으로 부터 읽은 데이터를 바이트어레이에 저장
    for(int i = 0; i < 11; i++)
    {
        bytearray[i] = clientSocket->read(BLOCK_SIZE);
    }

    Patient_Info type;
    char data[11][1020];
                                                                    // 데이터를 저장할 배열을 0으로 초기화
    for(int i = 0; i < 11; i++)
    {
        memset(data[i], 0, 1020);
        QDataStream in(&bytearray[i], QIODevice::ReadOnly);                                            // 바이트어레이를 읽기전용으로 설정

        in.device()->seek(0);                                                                       // 처음 위치로 감
        in >> type;                                                                                 // 소켓으로 부터 타입을 읽음
        in.readRawData(data[i], 1020);                                                                 // 소켓으로 부터 데이터 읽음
    }
...
{
    Gender = ui->Patlent_Gender_Male_pushButton->text();
}
void Nurse::Patient_Gender_Female_pushButton_clicked()
{
    Gender = ui->Patlent_Gender_Female_pushButton->text();
}
void Nurse::Patient_Gender_None_pushButton_clicked()
{
    Gender = ui->Patlent_Gender_None_pushButton->text();
}
