    ...
    ui(new Ui::ServerForm), totalSize(0), byteReceived(0)
{
    ...
    connect(inviteAction, SIGNAL(triggered()), SLOT(inviteClient()));                           // 초대액션 누를경우 inviteClinet 슬롯 발생

    QAction* removeAction = new QAction(tr("&Kick out"));                                       // 강퇴액션 생성
    connect(removeAction, SIGNAL(triggered()), SLOT(kickOut()));                                // 강퇴액션 누를경우 kickOut 슬롯 발생
